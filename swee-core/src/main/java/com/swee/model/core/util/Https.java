...
public class Https {
    ...
    @SneakyThrows
    public static String doPutFrom(String url, Object object) {
        FormBody.Builder form = new FormBody.Builder();
        formAdd(object, form);
        okhttp3.RequestBody body = form.build();
        Request request = new Request.Builder().url(url).put(body).header("User-Agent", DISGUISED_USER_AGENT).build();
        Response response = client.newCall(request).execute();
        if (HttpStatus.OK.value() != response.code()) {
            throw new RuntimeException(response.message());
        }
    ...
    @SneakyThrows
    public static String doDeleteFrom(String url, Object object) {
        FormBody.Builder form = new FormBody.Builder();
        formAdd(object, form);
        okhttp3.RequestBody body = form.build();
        Request request = new Request.Builder().url(url).delete(body).header("User-Agent", DISGUISED_USER_AGENT)
                .build();
        Response response = client.newCall(request).execute();
        if (HttpStatus.OK.value() != response.code()) {
            throw new RuntimeException(response.message());
    ...
    private void formAdd(Object object, FormBody.Builder form) throws Exception {
        if (object instanceof Map) {
            Map<?, ?> map = (Map) object;
            for (Map.Entry<?, ?> entry : map.entrySet()) {
